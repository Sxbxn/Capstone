...
def delete1():
    ...
    if os.path.isdir(patch_input[:-1]):
        ...
        os.system('mkdir {}'.format(patch_input[:-1]))
...
def detection():
    ...
    os.system('python3 test.py --data data/test.yaml --img 512 --batch 16 --conf 0.001 --iou 0.65 --device cpu --cfg cfg/yolor_p6.cfg --weights runs/train/yolor_p6/weights/last.pt --name yolor_p6_val --names data/cell.names --save-txt --task test')
...
def merge():
    ...
    if os.path.isdir(merge_output[:-1]):
        ...
        os.system('mkdir {}'.format(merge_output[:-1]))
